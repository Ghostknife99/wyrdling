...
func _init() -> void:
	...
	var art = load("res://src/dungeon/scenery_art_v2.gd")
	var atlas: Texture2D = art.make_atlas()
	if atlas == null or atlas.get_width() != 512 or atlas.get_height() != 480:
		push_error("Scenery atlas failed to build at 512x480")
		quit(1)
		return
	print("OK    polished terrain atlas builds 512x480")

	var props: Texture2D = art.make_props_sheet()
	if props == null or props.get_width() != 256 or props.get_height() != 128:
		push_error("Scenery prop sheet failed to build at 256x128")
		quit(1)
		return
	print("OK    polished scenery prop sheet builds 256x128")
	...
	if tileset.tile_size != Vector2i(32, 32):
		push_error("Polished terrain source lost 32px TileSet size")
		quit(1)
		return
